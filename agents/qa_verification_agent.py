        ...
        for db_type, db_id in self.actual_database_ids.items():
            ...
            try:
                ...
                response = requests.get(
                    f"https://api.notion.com/v1/databases/{db_id}", headers=self.headers
                )
                ...
                results["details"][db_type] = {"exists": False, "error": str(e)}
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if record_count >= expected_count and genesis_found:
                        ...
                        print(
                            f"   ❌ {db_type}: {record_count} records (expected {expected_count}), Genesis Cloud: {'found' if genesis_found else 'NOT found'}"
                        )
                        results["errors"].append(
                            f"{db_type}: Missing or insufficient Genesis Cloud data"
                        )
        ...
        for record in records:
            ...
            if db_type == "accounts":
                ...
                if name:
                    ...
                    if any(
                        genesis_name in contact_name
                        for genesis_name in ["Stefan", "Julia", "Eyitayo"]
                    ):
                        return True
...
if __name__ == "__main__":
    ...
    qa_agent.verify_five_phase_workflow_complete(
        "/Users/chungty/Projects/vdg-clean/abm-research/genesis_cloud_comprehensive_research.json"
    )
