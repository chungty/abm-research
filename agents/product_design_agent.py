...
class ProductDesignAgent:
    ...
    def validate_skill_compliance(self) -> Dict:
        ...
        validation_results["overall_compliance"] = self._calculate_overall_compliance(
            validation_results
        )
            ...
            try:
                ...
                for phase_key, function_name in phases_to_check:
                    if function_name in content:
                        ...
                        print(
                            f"      ❌ {phase_key.replace('_', ' ').title()} - Missing function: {function_name}"
                        )

            except Exception as e:
                print(f"      ⚠️ Error reading ABM research file: {e}")
                validation_results["missing_components"].append(
                    "ABM research implementation unreadable"
                )
        else:
            ...
            try:
                ...
                if "actual_database_ids" in content:
                    ...
                    for db_name in required_databases:
                        if (
                            f"'{db_name}':" in content
                            and "None" not in content.split(f"'{db_name}':")[1].split(",")[0]
                        ):
                    ...
                    validation_results["missing_components"].append(
                        "Real Notion database IDs not configured"
                    )
    ...
    def _validate_scoring_formulas(self, validation_results: Dict):
        ...
        if scoring_config_file.exists():
            try:
                ...
                expected_weights = scoring_config.get("scoring_formula", {}).get(
                    "component_weights", {}
                )

                if (
                    expected_weights.get("icp_fit_weight") == 0.40
                    and expected_weights.get("buying_power_weight") == 0.30
                    and expected_weights.get("engagement_weight") == 0.30
                ):
    ...
    def _generate_compliance_recommendations(self, validation_results: Dict) -> List[str]:
        ...
        for phase, compliant in validation_results["phase_compliance"].items():
            if not compliant:
                ...
                recommendations.append(
                    f"🔄 IMPLEMENT {phase_name} according to SKILL.md specification"
                )
        ...
        if not validation_results["scoring_formula_compliance"]:
            recommendations.append(
                "🧮 IMPLEMENT proper lead scoring formula (ICP:40%, Buying Power:30%, Engagement:30%)"
            )
    ...
    def _print_compliance_summary(self, validation_results: Dict):
        ...
        overall_status = (
            "✅ COMPLIANT" if validation_results["overall_compliance"] else "❌ NON-COMPLIANT"
        )
            ...
            "phases": {
                ...
                "phase_2": {
                    ...
                    "required_functions": [
                        ...
                        "initial_contact_scoring",
                        "segment_buying_committee",
                    ],
                ...
                "phase_4": {
                    ...
                    "required_functions": [
                        ...
                        "diagnose_connection_pathways",
                        "generate_value_add_ideas",
                    ],
                ...
                "phase_5": {
                    ...
                    "required_functions": [
                        "scan_vendor_relationships",
                        "document_partnerships",
