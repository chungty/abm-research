...
class ABMFinanceAgent:
    ...
    def analyze_recent_research_session(self) -> Dict:
        ...
        print("=" * 60)
        print(
            "📈 Based on actual 7-company research: 473.2s, 36 contacts, 7 events, 11 partnerships"
        )
        ...
        efficiency_metrics = self._calculate_session_efficiency_metrics(
            session_data, cost_breakdown
        )
    ...
    def _calculate_actual_session_costs(self, session_data: Dict) -> Dict:
        ...
        estimated_costs = {
            ...
            "apollo_enrichment": contacts
            * self.api_costs["apollo_enrichment"],  # Enrich each contact
            "openai_processing": companies
            * 20
            * self.api_costs["openai_gpt4"],  # ~20k tokens per company
            "brave_searches": companies
            * 3
            * self.api_costs["brave_search"],  # 3 searches per company
            "linkedin_scraping": min(contacts, 25)
            * self.api_costs["linkedin_scraping"],  # Top contacts only
            "notion_operations": contacts
            * 5
            * self.api_costs["notion_operations"],  # 5 operations per contact
            "rate_limit_penalty": rate_limit_hits
            * 0.10,  # Estimated delay costs from rate limiting
        }
    ...
    def _calculate_session_efficiency_metrics(
        ...
        return {
            ...
            "research_speed_multiplier": round(
                150 / max(duration_hours, 0.1), 1
            ),  # vs manual research time
            ...
            recommendations.append(
                {
                    ...
                    "potential_savings": round(
                        detailed_costs.get("rate_limit_penalty", 0) + 1.0, 3
                    ),
    ...
    def calculate_monthly_projections(self, companies_per_month: int = 50) -> Dict:
        ...
        optimization_savings = sum(
            rec["potential_savings"] for rec in current_analysis["optimization_recommendations"]
        )
        monthly_optimization_savings = optimization_savings * companies_per_month

        optimized_monthly = {
            "optimized_automated": monthly_costs["automated_research"]
            - monthly_optimization_savings,
            "additional_savings": monthly_optimization_savings,
            "total_monthly_savings": monthly_costs["monthly_savings"]
            + monthly_optimization_savings,
        }
    ...
    def generate_comprehensive_finance_report(self) -> Dict:
        ...
        comprehensive_report = {
            "executive_summary": {
                ...
                "research_speed_multiplier": session_analysis["efficiency_metrics"][
