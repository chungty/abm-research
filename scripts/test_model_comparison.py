...
def test_model(model: str, prompt: str) -> dict:
    ...
    response = client.chat.completions.create(
        model=model,
        messages=[
            {"role": "system", "content": "You are a B2B sales intelligence analyst. Extract structured data from text."},
...
def run_comparison():
    ...
    print("-" * 75)
    print(f"{'Total Cost':<30} ${gpt4_total_cost:.6f}     ${mini_total_cost:.6f}     {(1 - mini_total_cost/gpt4_total_cost)*100:.1f}%")
    print(f"{'Avg Response Time (ms)':<30} {gpt4_total_time/total_samples:.0f}           {mini_total_time/total_samples:.0f}           {(1 - mini_total_time/gpt4_total_time)*100:.1f}%")
    print(f"{'Quality Match Rate':<30} {'baseline':<15} {extraction_matches/total_samples*100:.0f}%")
