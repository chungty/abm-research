...
class ApolloUsageMonitor:
    ...
    def clean_old_data(self):
        ...
        if old_days or old_hours or expired_keys:
            ...
            self.save_cache_data()
            print(
                f"🧹 Cleaned {len(old_days)} old days, {len(old_hours)} old hours, {len(expired_keys)} expired cache entries"
            )
...
class SafeApolloAPI:
    ...
    def search_people_safe(self, company_domain: str, limit: int = 25) -> List[Dict]:
        ...
        # Make API call
        print(
            f"🔍 Apollo: Making new API call for {company_domain} (Daily: {limits['daily_used']}/{self.usage_monitor.daily_limit})"
        )
