...
TARGET_TITLES = ['DevOps', 'SRE', 'Infrastructure', 'Cloud', 'Platform', 'Director', 'Manager', 'VP', 'CTO', 'CFO', 'CEO', 'Founder', 'Head of']

# Known vendors for partnership discovery
KNOWN_VENDORS = ['NVIDIA', 'AMD', 'Intel', 'Schneider Electric', 'Vertiv', 'Equinix', 'AWS', 'Google Cloud', 'Microsoft Azure']
...
async def search_apollo_contacts(domain, limit=5):
    ...
    payload = {'q_organization_domains': domain, 'page': 1, 'per_page': limit, 'person_titles': TARGET_TITLES}
...
async def search_brave_vendor(company_name, vendor_name):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=headers, params={'q': query, 'count': 5}, timeout=15) as response:
                if response.status == 200:
                    data = await response.json()
