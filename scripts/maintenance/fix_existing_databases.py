#!/usr/bin/env python3
"""
Fix and Enhance Existing Notion Databases
Smart enhancement that works with existing fields and fixes API validation issues
"""
...
def enhance_contacts_database(database_id: str, accounts_db_id: str):
    ...
    if "LinkedIn Activity Level" not in current_props:
        required_fields["LinkedIn Activity Level"] = {
            "select": {
                "options": [
    ...
    if "Network Quality" not in current_props:
        required_fields["Network Quality"] = {
            "select": {
                "options": [
    ...
    if "Apollo Contact ID" not in current_props:
        required_fields["Apollo Contact ID"] = {"rich_text": {}}

    if "Apollo Person ID" not in current_props:
        required_fields["Apollo Person ID"] = {"rich_text": {}}

    if "Created At" not in current_props:
...
def main():
    ...
    print("🚀 FIXING AND ENHANCING EXISTING ABM DATABASES")
    print("=" * 60)
    ...
    config_content = f"""# Fixed ABM Database Configuration
