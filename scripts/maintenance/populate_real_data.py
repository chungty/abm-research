    ...
    for i, company in enumerate(target_companies, 1):
        ...
        try:
            ...
            if research_results.get('research_summary', {}).get('status') == 'failed':
                print(f"❌ Research failed for {company['name']}: {research_results['research_summary'].get('error', 'Unknown error')}")
                failed_researches += 1
            else:
                print(f"✅ Research completed for {company['name']} in {research_duration:.1f} seconds")
