...
class TestAuthentication:
    ...
    def test_api_research_requires_auth(self, client):
        ...
        response = client.post(
            "/api/research/start", json={"company_name": "Test", "domain": "test.com"}
        )
        ...
        if response.status_code == 200:
            ...
            for pattern in api_key_patterns:
                assert not re.search(
                    pattern, response_str, re.IGNORECASE
...
class TestInputValidation:
    ...
    def test_xss_prevention_in_company_name(self, client):
        ...
        for payload in xss_payloads:
            response = client.post(
                "/api/research/start", json={"company_name": payload, "domain": "test.com"}
            )
    ...
    def test_sql_injection_prevention(self, client):
        ...
        for payload in sql_payloads:
            response = client.post(
                "/api/research/start", json={"company_name": "Test", "domain": payload}
            )
    ...
    def test_domain_validation(self, client):
        ...
        for domain in invalid_domains:
            response = client.post(
                "/api/research/start", json={"company_name": "Test", "domain": domain}
            )

            # Should reject invalid domains
            assert response.status_code == 400, f"Invalid domain accepted: {domain}"
...
class TestRateLimiting:
    ...
    def test_api_data_rate_limit(self, client):
        ...
        assert 429 in unique_codes or all(
    ...
    def test_research_endpoint_rate_limit(self, client):
        ...
        for i in range(10):
            response = client.post(
                "/api/research/start", json={"company_name": f"Test{i}", "domain": f"test{i}.com"}
            )
            responses.append(response.status_code)
        ...
        unique_codes = set(responses)
        assert (
            429 in unique_codes or len(responses) < 10
...
class TestErrorHandling:
    ...
    def test_cors_configuration(self, client):
        ...
        response = client.get("/api/data", headers={"Origin": "http://malicious.com"})
...
class TestDataLeakage:
    ...
    def test_no_debug_mode_in_production(self, app):
        ...
        assert not app.debug or app.config.get(
            "TESTING"
    ...
    def test_no_sensitive_config_exposed(self, client):
        ...
        debug_endpoints = ["/debug", "/config", "/_debug", "/api/config", "/api/debug"]

        for endpoint in debug_endpoints:
            response = client.get(endpoint)
            # Should return 404, not configuration data
            assert response.status_code == 404, f"Debug endpoint {endpoint} should not exist"
...
class TestDoSPrevention:
    ...
    def test_large_payload_rejection(self, client):
        ...
        response = client.post(
            "/api/research/start", json={"company_name": large_string, "domain": "test.com"}
        )

        # Should reject or handle gracefully
        assert response.status_code in [400, 413, 500], "Large payloads should be rejected"
    ...
    def test_concurrent_request_handling(self, client):
        ...
        assert all(
            code in [200, 429, 503] for code in results
