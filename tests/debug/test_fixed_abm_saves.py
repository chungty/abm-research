...
def test_fixed_abm_saves():
    ...
    try:
        ...
        if notion_persistence:
            ...
            print(
                f"   🏢 Account saved: {account_saved} ({'✅ SUCCESS' if account_saved else '❌ FAILED'})"
            )
            ...
            if account_saved:
                ...
                print(
                    "✅ The problem was using wrong field name 'saved_to_notion' instead of 'notion_persistence'"
                )
                return True
            else:
