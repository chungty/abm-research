...
class SchemaValidator:
    ...
    def validate_account_data(self, account_data: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        for field in self.enhanced_schema['account']['confidence_required_fields']:
            ...
            if value and not self._has_confidence_indicator(value):
                issues.append(f"Field '{field}' missing confidence indicator (use 'confidence%', 'not found (searched)', or 'N/A - not searched')")
    ...
    def validate_contact_data(self, contact_data: Dict[str, Any], account_name: str = "") -> Tuple[bool, List[str]]:
        """
        ...
        if not contact_data.get('account_id') and not account_name:
            issues.append("Contact missing account relation - must have account_id or account_name for proper linking")
        ...
        for field in self.enhanced_schema['contact']['data_provenance_fields']:
            if field in contact_data:
                ...
                if field.endswith('_source') and value not in ['apollo', 'linkedin', 'merged', 'manual', 'inferred']:
                    issues.append(f"Invalid {field} value: {value}. Must be one of: apollo, linkedin, merged, manual, inferred")
                elif field == 'data_quality_score' and (not isinstance(value, (int, float)) or value < 0 or value > 100):
    ...
    def validate_trigger_event_data(self, event_data: Dict[str, Any], account_name: str = "") -> Tuple[bool, List[str]]:
        """
        ...
        if not event_data.get('account_id') and not account_name:
            issues.append("Trigger event missing account relation - must have account_id or account_name for proper linking")
        ...
        for field in self.enhanced_schema['trigger_event']['tactical_intelligence_fields']:
            ...
            if value and not self._is_tactical_content(value):
                issues.append(f"Field '{field}' should contain tactical, time-bound content, not strategic content")
        ...
        for field in self.enhanced_schema['trigger_event']['time_intelligence_fields']:
            ...
            if field.endswith('_deadline') or field.endswith('_window'):
                ...
                if value and value not in ['Fast', 'Medium', 'Slow', 'Permanent']:
                    issues.append(f"Decay Rate must be one of: Fast, Medium, Slow, Permanent, got: {value}")
            elif field == 'event_stage':
                if value and value not in ['Rumored', 'Announced', 'In-Progress', 'Completed']:
                    issues.append(f"Event Stage must be one of: Rumored, Announced, In-Progress, Completed, got: {value}")
    ...
    def validate_partnership_data(self, partnership_data: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        strategic_fields_present = sum(1 for field in self.enhanced_schema['partnership']['strategic_intelligence_fields']
                                     if partnership_data.get(field))
        if strategic_fields_present < 3:
            issues.append(f"Partnership should have at least 3 strategic intelligence fields populated, found: {strategic_fields_present}")

        # Validate specific strategic intelligence field values
        relationship_depth = partnership_data.get('relationship_depth', '')
        if relationship_depth and relationship_depth not in ['Surface Integration', 'Go-to-Market Alliance', 'Strategic Investment', 'Ecosystem Play']:
        ...
        if partnership_maturity and partnership_maturity not in ['Basic', 'Intermediate', 'Sophisticated']:
            issues.append(f"Invalid Partnership Maturity: {partnership_maturity}")

        best_approach = partnership_data.get('best_approach', '')
        if best_approach and best_approach not in ['Technical Discussion', 'Business Development', 'Channel Partnership']:
            issues.append(f"Invalid Best Approach: {best_approach}")

        decision_timeline = partnership_data.get('decision_timeline', '')
        if decision_timeline and decision_timeline not in ['Fast (weeks)', 'Medium (months)', 'Slow (quarters)']:
            issues.append(f"Invalid Decision Timeline: {decision_timeline}")

        return len(issues) == 0, issues

    def validate_complete_research_result(self, result: Dict[str, Any]) -> Tuple[bool, Dict[str, List[str]]]:
        """
        ...
        if account_name and len(contacts) > 0:
            ...
            if inconsistent_contacts:
                all_issues['general'].append(f"Contacts linked to different account than main account: {inconsistent_contacts}")
    ...
    def _has_confidence_indicator(self, value: str) -> bool:
        ...
        for term in self.confidence_patterns['high_confidence_terms'] + self.confidence_patterns['low_confidence_terms']:
    ...
    def _is_strategic_content(self, value: str) -> bool:
        ...
        tactical_indicators = [
            'deadline', 'by next week', 'urgent', 'immediate action',
            'follow up', 'schedule call', 'contact by', 'respond within'
        ]
    ...
    def _is_tactical_content(self, value: str) -> bool:
        ...
        tactical_indicators = [
            'action', 'deadline', 'follow up', 'schedule', 'contact',
            'next step', 'within', 'by', 'urgent', 'immediate',
    ...
    def generate_validation_report(self, result: Dict[str, Any]) -> str:
        ...
        for category, issues in issues_by_category.items():
            if issues:
                ...
                report.append(f"{category_emoji} {category.upper().replace('_', ' ')} ISSUES ({len(issues)}):")
