...
class ContactValueAnalyzer:
    ...
    def _analyze_single_contact_value(self, contact: Dict) -> ContactValue:
        ...
        buying_power = (
            contact.get("buying_power_score", 0) / 100.0
            if contact.get("buying_power_score")
            else 0.3
        )
        engagement_potential = (
            contact.get("engagement_potential_score", 0) / 100.0
            if contact.get("engagement_potential_score")
            else 0.4
        )
        ...
        elif linkedin_activity in ["Medium", "High"] or research_status in [
            "Basic Profile",
    ...
    def _analyze_organizational_patterns(
        ...
        company_name = (
            company_data.get("name", "Unknown Company") if company_data else "Unknown Company"
        )
        ...
        decision_makers = [
            ...
            if cv.role_analysis.buying_influence == "Decision Maker"
        ][:5]
        champions = [
            cv.name for cv in contact_values if cv.role_analysis.buying_influence == "Champion"
        ][:3]
    ...
    def _identify_role_patterns(self, contact_values: List[ContactValue]) -> List[RolePattern]:
        ...
        energy_contacts = [
            cv
            for cv in contact_values
            if "energy" in cv.role_analysis.title.lower() or cv.role_analysis.department == "Energy"
        ]
        if energy_contacts:
            patterns.append(
                RolePattern(
    ...
    def _generate_approach_recommendation(
        self, role: ContactRole, tier: str, readiness: str
    ) -> str:
    ...
    def _calculate_summary_metrics(self, contact_values: List[ContactValue]) -> Dict:
        ...
        decision_makers = len(
            [cv for cv in contact_values if cv.role_analysis.buying_influence == "Decision Maker"]
        )

        avg_value_score = sum(cv.final_value_score for cv in contact_values) / total_contacts
        avg_verdigris_relevance = (
            sum(cv.role_analysis.verdigris_relevance for cv in contact_values) / total_contacts
        )
        ...
        return {
            ...
            "engagement_opportunity_score": round(
                (tier_1_count + tier_2_count) / total_contacts * 100, 1
            ),
        }
    ...
    def _print_value_analysis(self, analysis: Dict):
        ...
        print(
            f"🎯 Tier 1 Contacts: {org_insights['tier_1_contacts']} ({metrics['tier_1_percentage']:.1f}%)"
        )
        print(
            f"📈 Tier 2 Contacts: {org_insights['tier_2_contacts']} ({metrics['tier_2_percentage']:.1f}%)"
        )
        ...
        print(
            f"   Champions: {', '.join(org_insights['champions_identified'][:2]) if org_insights['champions_identified'] else 'None identified'}"
        )
        ...
        print(
            f"   • {metrics['engagement_opportunity_score']:.1f}% of contacts are viable prospects"
        )
