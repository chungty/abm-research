...
class EnhancedAccountPlanGenerator:
    ...
    def generate_enhanced_account_plan(
        self, account_id: str = None, company_name: str = None
    ) -> EnhancedAccountPlan:
        """
    ...
    def _gather_signals_data(self, account_id: str, company_name: str) -> List[Dict]:
        ...
        try:
            ...
            if events:
                account_data = self._gather_account_data(account_id, company_name)
                enhanced_signals = self.signals_analyzer.analyze_buying_signals(
                    events, account_data
                )
    ...
    def _analyze_icp_composition(
        ...
        found_roles = {
            "Decision Maker": len(decision_makers),
            "Influencer": len(influencers),
            "Champion": len(champions),
        }

        coverage_score = sum(
            min(found_roles[role], required) for role, required in required_roles.items()
        ) / sum(required_roles.values())

        # Identify missing roles
        missing_roles = [
            role for role, required in required_roles.items() if found_roles[role] < required
        ]
    ...
    def _find_relevant_contacts_for_signal(
        ...
        high_value_contacts = [
            c for c in contacts_data if c.get("value_tier") in ["Tier 1", "Tier 2"]
        ]
        ...
        for contact in high_value_contacts:
            ...
            # Department matching
            if "energy" in signal_description and department in [
                "operations",
                "facilities",
                "sustainability",
            ]:
            ...
            if any(
                keyword in verdigris_relevance for keyword in ["energy", "monitoring", "efficiency"]
            ):
        ...
        if not relevant_contacts and high_value_contacts:
            ...
            decision_maker = next(
                (
                    c
                    for c in high_value_contacts
                    if c.get("role_analysis", {}).get("buying_influence") == "Decision Maker"
                ),
                None,
            )
            if decision_maker:
            ...
            for contact in tier_1_contacts[:3]:  # Top 3 Tier 1 contacts
                action = RecommendedAction(
                    ...
                    success_metrics=[
                        ...
                        "Technical evaluation initiated",
                        "Budget discussion",
                    ],
                    timeline="1-2 weeks",
    ...
    def _develop_account_strategy(
        ...
        if decision_makers >= 3:
            ...
            "operations" in c.get("role_analysis", {}).get("department", "").lower()
            for c in contacts_data
        ):
        ...
        if signals_data and any("energy" in s.get("description", "").lower() for s in signals_data):
            primary_value_prop = (
                "Energy cost reduction and operational efficiency through intelligent monitoring"
            )
        elif signals_data and any(
            "sustainability" in s.get("description", "").lower() for s in signals_data
        ):
            primary_value_prop = (
                "ESG compliance and sustainability metrics through comprehensive energy analytics"
            )
        else:
