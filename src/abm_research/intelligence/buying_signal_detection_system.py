...
class NewsMonitor:
    ...
    def _analyze_news_for_triggers(
        ...
        keyword_matches = sum(
            1
            for keywords in self.trigger_keywords[event_type]
...
class BuyingSignalDetection:
    ...
    def _prioritize_signals(self, signals: List[TriggerEvent]) -> List[TriggerEvent]:
        ...
        for signal in signals:
            ...
            priority_score = (
                signal.relevance_score * 0.4 + signal.confidence_score * 0.3 + urgency_weight * 10
            )
...
def main():
    ...
    for i, signal in enumerate(results["signals"][:3], 1):
        ...
        print(
            f"      Relevance: {signal.relevance_score:.0f}% | Confidence: {signal.confidence_score:.0f}%"
        )
