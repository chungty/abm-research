...
class ABMDataQualitySystem:
    ...
    def __init__(self):
        ...
        # Data quality rules
        self.quality_rules = [
            DataQualityRule("duplicate_accounts", "Check for duplicate accounts", "critical", "check_duplicate_accounts", True),
            DataQualityRule("duplicate_contacts", "Check for duplicate contacts", "critical", "check_duplicate_contacts", True),
            DataQualityRule("orphaned_contacts", "Check for contacts without accounts", "warning", "check_orphaned_contacts", True),
            DataQualityRule("incomplete_trigger_events", "Check trigger events missing fields", "warning", "check_incomplete_trigger_events", True),
            DataQualityRule("broken_relationships", "Check broken account-contact relationships", "critical", "check_broken_relationships", True),
            DataQualityRule("data_freshness", "Check data staleness", "info", "check_data_freshness", False),
            DataQualityRule("invalid_emails", "Check for invalid email formats", "warning", "check_invalid_emails", False),
            DataQualityRule("missing_linkedin_urls", "Check for missing LinkedIn URLs", "info", "check_missing_linkedin", False)
        ]
    ...
    def check_before_create_contact(self, contact_data: Dict) -> Optional[str]:
        ...
        if linkedin_url and linkedin_url in self.dedup_cache['contacts']:
            ...
            print(f"   ⚠️ Contact already exists (LinkedIn): {linkedin_url} -> {existing_id[:8]}...")
            return existing_id
        ...
        if name and title:
            ...
            if name_title_key in self.dedup_cache['contacts']:
                ...
                print(f"   ⚠️ Contact already exists (name+title): {name} {title} -> {existing_id[:8]}...")
                return existing_id
        ...
        for rule in self.quality_rules:
            ...
            try:
                ...
                if issues:
                    ...
                    print(f"      🔴 Found {len(issues)} issues (Fixed: {issue_result.get('auto_fixed', 0)})")
                else:
    ...
    def _get_total_record_count(self):
        return len(self._fetch_all_accounts()) + len(self._fetch_all_contacts()) + len(self._fetch_all_trigger_events())
