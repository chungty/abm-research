...
class ConfigManager:
    ...
    def _validate_configuration(self):
        ...
        if self.apollo_api_key and not self.apollo_api_key.startswith(("Ol", "sk-")):
            logger.warning(
                f"⚠️ Apollo API key format may be incorrect (starts with: {self.apollo_api_key[:3]}...)"
            )

        if self.notion_api_key and not self.notion_api_key.startswith(("ntn_", "secret_")):
            logger.warning(
                f"⚠️ Notion API key format may be incorrect (starts with: {self.notion_api_key[:3]}...)"
            )

        if self.openai_api_key and not self.openai_api_key.startswith("sk-"):
            logger.warning(
                f"⚠️ OpenAI API key format may be incorrect (starts with: {self.openai_api_key[:3]}...)"
            )
        ...
        for name, db_id in self.database_ids.items():
            if len(db_id) != 32:
                logger.warning(
                    f"⚠️ {name} database ID length incorrect: {len(db_id)} chars (expected 32)"
                )
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"ConfigManager(apollo_key={'✓' if self.apollo_api_key else '✗'}, "
