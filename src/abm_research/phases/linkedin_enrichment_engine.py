...
class LinkedInEnrichmentEngine:
    ...
    def load_skill_config(self):
        ...
        self.responsibility_keywords = self.skill_config["icp_fit_scoring"]["components"][
            "responsibility_keywords"
        ]["keywords"]
        self.activity_scoring = self.skill_config["engagement_potential_scoring"]["components"][
            "linkedin_activity"
        ]["scoring"]
        self.content_scoring = self.skill_config["engagement_potential_scoring"]["components"][
            "content_relevance"
        ]["scoring"]
        self.network_scoring = self.skill_config["engagement_potential_scoring"]["components"][
            "network_quality"
        ]
    ...
    def enrich_high_priority_contacts(self, contacts: List[Dict]) -> List[Dict]:
        ...
        for contact in contacts:
            if contact.get("lead_score", 0) > 60:
                print(
                    f"🔍 Enriching LinkedIn data for {contact.get('name')} (Score: {contact.get('lead_score')})"
                )
                ...
                try:
                    ...
                    print(
                        f"✅ Enriched {contact['name']} - New score: {contact['final_lead_score']}"
                    )
    ...
    def enrich_contact(self, contact: Dict) -> Dict:
        ...
        try:
            ...
            enrichment_data = self.enrich_linkedin_profile(contact)

            # Update contact with enrichment data
            enriched_contact = self._update_contact_with_enrichment(contact, enrichment_data)

            # Recalculate final score with enrichment
            enriched_contact = self._recalculate_final_lead_score(enriched_contact)

            return enriched_contact

        except Exception as e:
            print(f"⚠️ LinkedIn enrichment failed for {contact.get('name', 'unknown')}: {e}")
    ...
    def enrich_linkedin_profile(self, contact: Dict) -> LinkedInEnrichmentData:
        ...
        engagement_score = (
            activity_analysis["points"] + content_analysis["points"] + network_analysis["points"]
        )
    ...
    def _fetch_real_linkedin_profile(self, contact: Dict, linkedin_url: str) -> Dict:
        ...
        try:
            ...
            if profile:
                ...
                print(
                    f"⚠️ Real data unavailable for {contact.get('name')}, using enhanced simulation"
                )
    ...
    def _format_connections_for_analysis(self, profile) -> List[Dict]:
        ...
        for skill in profile.skills[:5]:  # Use top 5 skills
            if any(
                ...
                for keyword in ["data center", "infrastructure", "power", "energy"]
            ):
                formatted_connections.append(
                    {
    ...
    def _analyze_bio_for_keywords(self, bio_text: str) -> Dict:
        ...
        max_bonus = self.skill_config["icp_fit_scoring"]["components"]["responsibility_keywords"][
            "max_bonus"
        ]
    ...
    def _analyze_linkedin_activity(self, recent_posts: List[Dict]) -> Dict:
        ...
        now = datetime.now()
        posts_last_7_days = len(
            [p for p in recent_posts if (now - datetime.fromisoformat(p["date"])).days <= 7]
        )
        posts_last_30_days = len(
            [p for p in recent_posts if (now - datetime.fromisoformat(p["date"])).days <= 30]
        )
        posts_last_90_days = len(
            [p for p in recent_posts if (now - datetime.fromisoformat(p["date"])).days <= 90]
        )
    ...
    def _assess_network_quality(self, connections: List[Dict]) -> Dict:
        ...
        for connection in connections:
            connection_text = (
                f"{connection.get('title', '')} {connection.get('company', '')}".lower()
            )
    ...
    def _analyze_connection_pathways(self, profile_data: Dict) -> Dict:
        ...
        for connection in connections[:10]:  # Analyze top 10 connections
            ...
            if any(
                company in connection.get("company", "").lower()
                for company in ["google", "microsoft", "amazon", "meta", "nvidia"]
            ):
                warm_intro_ops.append(
                    f"Connection at {connection.get('company')}: {connection.get('name')}"
                )
