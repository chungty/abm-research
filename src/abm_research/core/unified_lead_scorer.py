...
class AccountScorer:
    ...
    def calculate_account_score(self, account_data: Dict[str, Any]) -> AccountScore:
        ...
        total = (
            infrastructure.score * 0.35 + business_fit.score * 0.35 + buying_signals.score * 0.30
        )
    ...
    def _score_infrastructure(self, account_data: Dict[str, Any]) -> InfrastructureBreakdown:
        ...
        raw_text = (
            account_data.get("Physical Infrastructure", "")
            or account_data.get("physical_infrastructure", "")
            or account_data.get("current_tech_stack", "")
            or ""
        )
    ...
    def _score_business_fit(self, account_data: Dict[str, Any]) -> BusinessFitBreakdown:
        ...
        us_locations = [
            loc for loc in locations if any(ind in loc.lower() for ind in us_indicators)
        ]
    ...
    def _score_buying_signals(self, account_data: Dict[str, Any]) -> BuyingSignalsBreakdown:
        ...
        expansion_matches = [
            ind for ind in growth_indicators if any(kw in ind.lower() for kw in expansion_keywords)
        ]

        expansion_score = min(len(expansion_matches) * 30, 100)
        expansion_breakdown = {"detected": expansion_matches[:3], "score": expansion_score}
        ...
        hiring_matches = [
            ind for ind in growth_indicators if any(kw in ind.lower() for kw in hiring_keywords)
        ]

        hiring_score = min(len(hiring_matches) * 25, 100)
        hiring_breakdown = {"detected": hiring_matches[:3], "score": hiring_score}
...
class MEDDICContactScorer:
    ...
    def calculate_contact_score(
        ...
        total = champion_score * 0.45 + role_score * 0.30 + engagement_score * 0.25
...
class UnifiedLeadScorer:
            ...
            "icp_fit_scoring": {
                "components": {
                    ...
                    "responsibility_keywords": {
                        ...
                        "keywords": {
                            "infrastructure": {
                                "terms": ["infrastructure", "data center", "datacenter"],
                                "points": 8,
                            },
                            "power_energy": {
                                "terms": ["power", "energy", "efficiency"],
                                "points": 10,
                            },
                            "monitoring": {
                                "terms": ["monitoring", "analytics", "observability"],
                                "points": 7,
                            },
                            "operations": {
                                "terms": ["operations", "ops", "reliability"],
                                "points": 6,
                            },
                        },
                    },
    ...
    def calculate_comprehensive_lead_score(
        ...
        influence = self.decision_influence_map.get(
            role_classification, self.decision_influence_map["Unknown"]
        )
    ...
    def _score_icp_fit(self, contact: Dict) -> float:
        ...
        if bio:
            keywords = self.config["icp_fit_scoring"]["components"]["responsibility_keywords"][
                "keywords"
            ]
            bio_lower = bio.lower()
            ...
            max_bonus = self.config["icp_fit_scoring"]["components"]["responsibility_keywords"][
                "max_bonus"
            ]
    ...
    def _score_engagement_potential(self, contact: Dict) -> float:
        ...
        activity_config = self.config["engagement_potential_scoring"]["components"][
            "linkedin_activity"
        ]["scoring"]
        ...
        # Content relevance
        content_themes = contact.get("content_themes", [])
        high_relevance_topics = self.config["engagement_potential_scoring"]["components"][
            "content_relevance"
        ]["high_relevance_topics"]
        relevant_themes = [
            theme
        ...
        content_scores = self.config["engagement_potential_scoring"]["components"][
            "content_relevance"
        ]["scoring"]
        ...
        if contact.get("network_quality", False):
            score += self.config["engagement_potential_scoring"]["components"]["network_quality"][
                "score"
            ]
    ...
    def _score_geographic_fit(self, account_data: Dict) -> Tuple[float, GeographicPriority]:
        ...
        if us_data_centers:
            ...
            if international_data_centers:
                ...
                priority = GeographicPriority.INTERNATIONAL_ONLY
                red_flags.append(
                    f"No US data center presence detected. Locations: {', '.join(international_data_centers[:3])}"
                )
            else:
                score = 40
                priority = GeographicPriority.INTERNATIONAL_ONLY
        ...
        if employee_locations:
            ...
            if us_employees:
                ...
                red_flags.append(
                    f"No US employee presence detected. Employee locations: {', '.join(employee_locations[:3])}"
                )
        ...
        if not us_presence:
            red_flags.append(
                "⚠️ RED FLAG: No US market presence detected - may not align with Verdigris go-to-market focus"
            )
    ...
    def _calculate_organizational_influence_score(self, influence: DecisionInfluence) -> float:
        ...
        return (
            influence.entry_point_value * 0.4
            + influence.pain_ownership * 0.3  # PRIMARY: Best place to start?
            + influence.champion_ability * 0.2  # Do they feel the pain?
            + influence.budget_authority  # Can they sell internally?
            * 0.1  # SECONDARY: Can they approve?
        ) * 100  # Scale to 0-100
    ...
    def calculate_lead_score(self, contact: Dict, account_data: Dict = None) -> float:
        ...
        try:
            ...
            influence = self.decision_influence_map.get(
                role, self.decision_influence_map["Unknown"]
            )
